...
1. **Inconsistent document structure could make chunking uneven.** Most sources are Markdown support articles with headings, but some important files use standalone bold labels instead of real headings, and a few sections have blank or weak headings. If the chunker only splits on Markdown headings, documents like the Stage Screen, Macros, and Keyboard Shortcuts articles may stay too broad. If the chunker treats every bold line as a heading, it may over-split normal emphasis. I will need conservative soft-heading rules and size-based fallbacks so workflow steps, tables, and related explanations stay together.

2. **Relevant details may be split across related ProPresenter concepts.** Some user questions combine features that live in separate documents, such as Stage Screens, Looks, Screen Configuration, timers, and slide actions. For example, a lower-thirds issue may require retrieving the Looks article, while a confidence monitor question may use volunteer language that maps to Stage Screens. If `top_k` is too low or retrieval relies only on exact wording, the system may miss the source that contains the actual answer.

3. **Dense semantic retrieval may miss exact UI labels, keyboard shortcuts, and menu paths.** The MVP uses `all-MiniLM-L6-v2` with cosine similarity, which should work well for natural-language questions, but it may be weaker for exact strings like `Command+4`, `F1`, `Add Action > Audience Look`, or `M`, `S`, and `T` in the Audio Routing window. This could cause the system to retrieve a generally related article while missing the specific command or shortcut needed for a correct answer.

4. **The model may answer beyond the retrieved documents for vague or unrelated questions.** Volunteers may ask broad questions like "How do I fix my screens?" or unrelated questions like what camera to buy for livestreaming. Without a strict grounding prompt and low-relevance filtering, the model may fill gaps with general knowledge instead of saying the current ProPresenter documents do not contain enough information. The generation step needs to clearly prefer clarification or an out-of-scope response when retrieval is weak.

5. **Source attribution could become unclear if metadata is not preserved through the pipeline.** The documents include useful front matter such as title, source URL, document type, and product, and transcript sources may eventually need timestamp metadata. If ingestion or chunking drops this metadata, the final answers may be hard to verify and the Evaluation Report will be weaker because I will not be able to tell which source supported each response.
